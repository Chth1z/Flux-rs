...
- The shipped bootstrap `sing-box.json` now follows the original Flux module's
...
- `module/action.sh`, and with it the Magisk v28+ floor it implied. A dedicated
  ...
  button and the second switch file. The packaging allowlist is now 14 files.
...
WebUI, an out-of-the-box proxy control panel, and subscription conversion
(confirmation items C8, C10 and C11) are deferred to a later version by owner
decision on 2026-08-29 — deferred, not abandoned. 0.9.1 ships none of them and
pre-builds no seam for them.
