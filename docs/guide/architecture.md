...
`configuration` 是运行、诊断和安装共用的配置读取边界。`subscription` 管理有界的单个抓取批次和候选快照（批次墙钟 60s、总正文 8 MiB，不是每源 8 MiB）；reactor 只协调一次引擎事务。迟到结果按 fetch epoch 丢弃。缓存以准确 URL 的 SHA-256 命名，移除正文与 URL 标记的双文件一致性问题。只有接受后的原始响应才持久化，没有缓存索引或精修缓存。
