...
| `R09x-NN` | 某版本的增量修订项 | `spec/blueprint-0.9.x.md` | 该版本增量蓝图 | **已停止新增**；0.9.5 折叠后退役，由对照表映射到承载它的 §（AUTH-7.2） |
...
- **规范性变更就地改蓝图**，不新开增量层（AUTH-7.2）。0.9.5 折叠完成前，`spec/` 里暂存的两份 0.9.x 增量仍然有效。
...
| 主题 | 唯一真相源 |
|---|---|
| 产品合同 | `spec/blueprint.md`。0.9.5 折叠完成前还要叠 `blueprint-0.9.1.md` 与 `blueprint-0.9.2.md`，冲突时取最新一层；折叠后这两份移入 `history/`（AUTH-7.2） |
