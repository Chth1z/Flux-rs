...
| 前缀 | 含义 | 定义在 | 分配者 | 稳定性 |
|---|---|---|---|---|
| `§N[.N…]` | 蓝图部分与章节 | `spec/` 全部文件 | 蓝图作者 | 全局稳定，发布后不复用、不重排 |
| `R09x-NN` | 某版本的增量修订项 | `spec/blueprint-0.9.x.md` | 该版本增量蓝图 | 版本内稳定；版本冻结后不再新增 |
...
- **跨版本的修订不改旧编号**：0.9.1 用 `R091-*` 覆盖 0.9.0 的章节，被覆盖的原文保留。
...
被引用的编号必须真实存在。`doc-check` 会把悬空引用当作缺陷报出来。
...
| 主题 | 唯一真相源 |
|---|---|
| 产品合同 | `spec/blueprint.md` + `spec/blueprint-0.9.1.md` + `spec/blueprint-0.9.2.md`；冲突时取最新一层 |
