...
>
> **2026-09-06 进度。** 装卸、激活表、批次 B/D/E/F 的命令行断言已写入 [`../history/review-log.md`](../history/review-log.md) §0.6.6。三个缺口的处置：动态链接 Bionic、`PROXY`/`AUTO` 的 `DIRECT` 占位视为空位、`bugreport` 默认写状态根。仍未做：用新包重测订阅抓取与默认模板填组、WebUI 按钮、Phase 3–7 套件、Magisk/APatch。
>
> **2026-09-07 进度。** 日常使用中暴露并修复了一个 §26 不变量 4 级别的缺陷：离开 Wi-Fi 切蜂窝时 `tc_filter:ESTALE` 打死整个代理且不自愈（复现、根因与修复后实测见 §0.6.7）。`c6a3c6a` 的 ZIP（sha256 `566f10c8…`）已装回同一台设备并复验切换。本清单未新增条目：该缺陷不在表内，是真机日常使用发现的，这本身说明 §5 的 Phase 3–7 套件仍是必须跑的一步。仍未做：WebUI 按钮、Phase 3–7 套件、Magisk/APatch。
