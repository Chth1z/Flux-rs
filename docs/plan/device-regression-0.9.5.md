...
>
> **2026-09-07 进度。** 日常使用中暴露并修复了一个 §26 不变量 4 级别的缺陷：离开 Wi-Fi 切蜂窝时 `tc_filter:ESTALE` 打死整个代理且不自愈（复现、根因与修复后实测见 §0.6.7）。**§0 那个 ZIP 因此作废，下面每一步都要用重新打的包。** 本清单未新增条目：该缺陷不在表内，是真机日常使用发现的，这本身说明 §5 的 Phase 3–7 套件仍是必须跑的一步。
