...
>
> 谁读这份：要动手写代码、核对发布进度的人。现行合同只有 `../spec/blueprint.md` 及其指向的交互、失败规格；`history/blueprint-0.9.*.md` 是冻结记录。本文只管**当前进度、顺序、边界与验收**，不得反向创造产品要求。

---

## 17.0 当前实施状态（2026-09-16）

**按所有者确认的产品设计准备 1.0.0 候选。** 工作区版本为 `1.0.0-rc.1`，正式 1.0.0 仍需所有者审核和候选验收。基准为 `aa4715d`，审核分支为 `codex/v1.0.0-review`；本轮主要实现提交为 `baf15d8`。后续 §17.2–§17.12 保留阶段编号与回归方法，不是新的待办清单。
...
| 0.9.5 批次 A–F | 已实现；KernelSU 设备已验证订阅、开关、监督恢复、SSID 暂停及网络切换 | §0.6.6、§0.6.7；各批次任务书保留在 `../history/handoff-batch-*.md`。这些是旧构建的证据，不替代本候选复验 |
| 1.0.0 候选优化 | 加载器统一负责内核兼容性、JSONC 保留词法边界、Cargo 统一决定产物目录、移除时间格式化的反向依赖、用真实 BPF 产物替换过期数字断言、移除首次启用前的人工检查前置条件 | 设计判断与复现记录见 §0.6.8；实际改动以审核分支 diff 为准 |
| 本轮产品收敛 | **六批范围已实现，RC 软件复验与双构建完成，待所有者审核** | 构建提交 `160699a`；软件与产物证据见 §0.6.15–§0.6.17；实施范围见 §17.0.4 |
| 发布剩余 | **尚未具备正式发布结论** | 本候选的 §20 验收逐项状态见 §17.0.3 |

### 17.0.1 设计哲学返工清单

本轮新增要求按同一原则实施：手工节点与远端订阅进入同一节点池；抓取和名称清洗按职责分开；保持用户原有 outbound 菜单；将依赖版本选择交回上游，将实际解析结果留作构建证据。前一批优化已提交 `e9e09ad`，依据见 §0.6.8。完成项归档，不把历史删除线当作当前待办。

### 17.0.2 现行合同与当前实现的差距

上一批确认的实现缺口已处理。节点输入已提交 `1022e63`，刷新计划与订阅模块职责修正见 §0.6.10；版本解析、构建记录和网络设计见 §0.6.11–§0.6.12。本轮合同修订与六批实现范围见 §17.0.4，已完成对应实现和软件复验；证据见 §0.6.15–§0.6.17。**目前剩余的是所有者审核、远端 CI 与候选设备验收，不把历史设备通过当成本轮通过。**
...
**推荐保留当前产品路线，按下面的证据完成验收后发布。** 本轮以所有者重申的“优雅、简洁高效、从根本上避免问题”为取舍标准：优先消除重复真相、调用顺序约束和无效状态；不靠增加兜底分支或重复校验处理内部设计问题。对应 PHIL-1、PHIL-2、PHIL-4、PHIL-6。审查入口是本分支 diff、§10.2 的模块接口与 §0.6.8–§0.6.17 的复现记录。
...
| 3：官方引擎来源 | 干净提交 `160699a` 的 RC 解析 1.14.1，官方 host 检查、Android 资产校验和对应源码归档通过 | 正式版本按同一路径重新解析和构建 |
| 4：页大小 | 4 KiB 产品边界保留；fluxd 每个 LOAD 段要求 ≥ 0x4000 | 正式发布说明由所有者审核 |
...
| 8：系统共存 | 内核对象管理策略未改 | 候选设备前后枚举 diff 与 VPN/CLAT 断言 |
| 9：卸载与残留 | 真实宿主 fluxd + 合成 ZIP 验证安装、迁移、旧 Rust 身份退休及选择性解压 | 候选卸载并重启后的实际残留检查 |
| 10：三个管理器 | 只有历史 KernelSU 真机证据 | Magisk、KernelSU、APatch 各跑一次完整 smoke |
| 11：可复现打包 | `1.0.0-rc.1` 在 `/tmp` 下两个独立目标目录完整构建，ZIP 字节一致 | RC 哈希与范围见 §0.6.17；正式版本后复跑 |
| 12：DNS 精度 | 历史 Q9 记录可查 | 候选验证被选与未选应用的系统 DNS |
| 13：用户边界说明 | 首次启用由现有事务完成验证，`check` 用于诊断；控制面板告警已与合同对齐 | 所有者审核对外表述 |
| 14：实现差距 | 本轮六批设计已实现，软件复验与 RC 打包已完成 | 所有者审核最终分支及设备验证结果 |

本轮软件与产物验证以 §0.6.15–§0.6.17 为准。当前主机没有 `cargo-deny`，本轮未运行其供应链检查；最终分支还需远端 CI 的该项结果。**WSL 本地通过不等于远端 CI 已通过；旧设备记录不等于这个候选已通过。**

审核通过后的顺序：

1. 授权在测试设备安装候选并运行 `device-regression-0.9.5.md` 中仍适用的场景，补齐上表设备证据。
2. 在 workspace manifest 提升到 `1.0.0`，更新经审核的发布说明；不增加依赖版本锁。
3. 在最终提交上跑适用门禁和两次干净打包，核对原始源代码发布物、ZIP、SHA256SUMS 及构建标识。
4. 由所有者确定推送、签名 tag 和正式发布时机。

设备授权依据 GOV-1.2：安装模块、重启和设备状态变更需要所有者决定。当前工作只准备可审核的本地候选，不把授权和验收写成已完成。

---

### 17.0.4 本轮执行顺序与交接

**主代理负责设计、实现与验收。** 所有者最初指定 Cursor 的 Grok 4.6 Extra High 执行代码；桌面控制初始化失败且官方 CLI 下载返回 HTTP 403 后，所有者授权主代理自行执行。没有通过 Cursor 派发代码任务，也不将当前执行者表述为该模型。

本表是合同的实施顺序，不是第二份规范。每一批同步对应条款、代码和用户指南；每批通过 AGENTS.md 的宿主检查后提交，Linux/Android 相关改动另跑 §15.1 的适用检查。原始 `module/template.json` 保持逐字节不变。

| 顺序 | 实施范围 | 合同位置 | 必须证明的行为 |
|---|---|---|---|
| 1 | 主配置与进阶配置的纯解析、明确来源类型、预编译清洗/分组规则 | §11.2、§11.2.4、§28.2 | 手工/远端/混合输入共用接口；HTTP 节点不被当作订阅；手工名保留；缺失主文件不扩大接管；无效进阶项不会部分生效 |
| 2 | 多来源采集与来源缓存，减少 reactor 的订阅分支状态 | §10.2、§28.5–§28.7、§29.3–§29.4 | 来源换序复用缓存；URL 改变不串用；迟到结果不复活已删除来源；一个来源失败仍尝试其它来源；失败不取消刷新；无效候选保留当前代；相同结果不重启 |
| 3 | 运行目录、默认引擎缓存路径、写入时日志轮转 | §9.6、§11.1、§11.2.4、§27.2.1 | 默认产物归 `run/`；显式模板路径与所有 outbounds 保留；不删除活锁；长期运行也执行日志保留策略 |
| 4 | Rust 安装迁移与选择性解压 | §13.1、§13.2.3 | 首装停用；升级保留选择与模板；旧隐式 whitelist 被保留；结构迁移保留值，重复执行及两次发布之间中断可恢复；正常升级不重写现行配置；发行材料不成为运行依赖 |
| 5 | `Flux-rs` 身份与旧版风格的状态投影 | §1.1、§27.1.3 | 模块路径与 ID 一致；状态来自既有状态机；黑名单显示排除清单含义；PID 存在不等于 RUNNING；重复渲染不增长、不重复写盘 |
| 6 | 版本与产物身份、指南和治理收尾 | §13.4、GOV-7、GOV-8 | RC 顺序在正式版之前；算术溢出明确失败；开发包带提交身份；正式发布取干净提交；构建记录不变成依赖锁；指南与最终行为一致 |

优先通过模块接口测试这些行为，避免给每个内部 helper 配镜像测试。来源的解析、身份与有序组装留在纯逻辑层；I/O、事件通知与文件提交各有一个所有者。多来源不复制多套 timer、worker 或 engine 事务。新的日志策略由 logger 自己执行，避免 reactor 持有第二份文件轮转状态。

PHIL-10 的本轮设计复核：

- 开放的进阶字段都是用户策略；机制参数仍内部生成。
- 两份配置的字段归属互斥，来源使用明确类型；没有跨文件覆盖优先级。
- 缓存路径由来源身份派生，消除 URL 标记与正文的双文件提交。
- 使用现有配置事件、route 事件和单个刷新计时器；日志轮转由写入触发。
- 运行态不写用户配置；安装迁移有独立、一次性的职责与提交顺序。
- 默认引擎缓存路径通过生成阶段补全；用户模板及明确路径保留。
- 继承旧版配置保留与状态表达的意图，运行逻辑使用现有 Rust 模块。

现行 README 和指南已同步统一来源、进阶配置、迁移、运行目录与状态展示。Windows 宿主检查、Linux workspace Clippy 与完整测试、Android all-targets Clippy、宿主安装生命周期及两次干净打包均通过；记录见 §0.6.15–§0.6.17。本轮没有校园网或设备测试环境，不将这些软件结果外推为校园网故障已修复或新设备已通过。
