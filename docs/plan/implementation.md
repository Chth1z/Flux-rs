...
## 17.0 当前实施状态（2026-09-15）

**正在准备由所有者审核的 1.0.0 候选。** 工作区版本暂保留原值；审核、设备验收和发布门禁通过后才提升 workspace 版本并生成正式发布物。基准为 `aa4715d`，审核分支为 `codex/v1.0.0-review`。后续 §17.2–§17.12 保留阶段编号与回归方法，不是新的待办清单。
...
本轮确认的实现缺口已处理。节点输入已提交 `1022e63`，刷新计划与订阅模块职责修正见 §0.6.10；版本解析、构建记录和网络设计见 §0.6.11–§0.6.12。最终软件与双构建复验见 §0.6.13。剩余工作是所有者审核与设备验收，状态集中在下表。
