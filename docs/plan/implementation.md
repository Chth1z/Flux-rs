...
| 本轮产品收敛 | **设计已写入现有合同，实现尚未开始** | 所有者确认 `id=Flux-rs`、统一节点来源、进阶配置、增量升级、运行目录与展示方向；执行顺序见 §17.0.4 |
...
| 14：实现差距 | 上一批已完成；本轮新增设计待实现，见 §17.0.4 | 完成各批实现、审核与验证，再审核最终分支 |

其余软件门禁以 §0.6.13 记录的当次结果为准。**WSL 本地通过不等于远端 CI 已通过；旧设备记录不等于这个候选已通过。**
...
第一批代码完成前，现有 README/指南仍描述上一批已实现的接口；实现每批时同步相关用户示例，避免先把尚不能运行的配置当成现有操作步骤。正式候选不得保留这种差距。
