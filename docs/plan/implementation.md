...
当前没有未完成的哲学返工项。0.9.5 批次处置保留在历史任务书和 §0.6；1.0.0 这轮的判据、取舍与依据见 §0.6.8。后续发现的新项在此登记，完成后归档，不能把历史删除线当作当前待办。

### 17.0.2 现行合同与当前实现的差距

当前没有已确认、尚未修复的代码差距。已完成的 0.9.5 合同追赶由 `../history/handoff-batch-*.md` 记录；本轮更正在 §0.6.8。这里为空只说明已知实现缺口已处理，**不表示设备验收已通过**。

### 17.0.3 1.0.0 候选审核与发布验收

**推荐保留当前产品路线，按下面的证据完成验收后发布。** 本轮以所有者重申的“优雅、简洁高效、从根本上避免问题”为取舍标准：优先消除重复真相、调用顺序约束和无效状态；不靠增加兜底分支或重复校验处理内部设计问题。对应 PHIL-1、PHIL-2、PHIL-4、PHIL-6。审查入口是本分支 diff、§10.2 的模块接口与 §0.6.8 的复现记录。
...
| 3：官方引擎 pin | 本轮打包验证 archive/binary size、SHA-256 与四个 LOAD 段 | 最终版本重打包时重新验证 |
...
| 14：实现差距 | §17.0.2 当前为空 | 审核过程中发现的问题先修复再发布 |
...
2. 在 workspace manifest 提升到 `1.0.0`，更新对应锁文件与经审核的发布说明。
