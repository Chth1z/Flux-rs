...
## 17.0 当前实施状态（2026-09-18）

**1.0.0-rc.4 把「如何把原包交给官方 sing-box」做成唯一数据面。** 工作区版本仍是 `1.0.0-rc.3`，直到 rc.4 命名批。rc.3 批 0–7 已落地。所有者 2026-09-18 锁定 C13：捕获钩子改为 `NF_INET_LOCAL_OUT` 的 `.ko`，`insmod` 失败则 Direct，不回退 veth。目标形状与批次见 `docs/plan/rc4.md`。正式 1.0.0 改到 **rc.4 完成之后** 再走远端 CI、候选 ZIP 的 §20、Phase 7 与三管理器 smoke。不实现 TCX、WebUI、默认控制面、校园网自动旁路；不把 iptables TPROXY 当第二条热路径。
...
| 1.0.0-rc.4 | 批 0–6 通过；批 6b 对齐 UID 1024 与 `SET_BYPASS`；批 7 命名未做 | `docs/plan/rc4.md`；§0.6.32–0.6.46。S6 为本机 dst 投递。默认 `FLUXRS_STAGE=6` `FLUXRS_NOCFI=1`。`dataplane::Manager` 加载 `.ko`；取消勾选拆活 TCP；信封含 GKI 代模块；CIDR/self-addr 经 ioctl 进钩子 |
...
rc.2 按 PHIL-1/2/4 与蓝图 §5：不为单一实现预建 trait；非法生命周期组合不可构造；`module.prop` 与 JSON `state` 共用 `CommittedView`；世代只有 `next` + `committed` 一个所有者。完成项归档，不把历史删除线当作当前待办。

### 17.0.2 现行合同与当前实现的差距

rc.1 的产品缺口已处理。本轮把 §26 收成纯函数、reactor 降为 epoll 适配器、clsact attach 收成模块、ingress 已建立/分片走 I1b。校园网仍只有现象记录，没有现场根因，不写进本版合同。I1b 已在 SM-S9180 / 5.15.211 经 Phase 4 `Runtime::load_embedded` 过 verifier；远端 CI 与候选 ZIP 的 §20 仍是正式 1.0.0 的签署门。

### 17.0.3 1.0.0 候选审核与发布验收

**签署放在 rc.4 之后。** 取舍标准不变：优先消除重复真相和无效状态。远端 CI、候选 ZIP 真机 §20、三管理器 smoke 仍是发布门，不是 rc.4 各批的设计范围。WSL 本地通过不等于远端 CI；旧设备记录不等于这个候选已通过。rc.3 的控制面加深保留，不在 rc.4 拆掉重做。
...
| 5：未选 UID 热路径 | E1 未改 | 候选真机短时 sanity check |
...
1. 授权在测试设备安装候选，补齐上表设备证据。本树 I1b 已在 5.15.211 过 verifier；候选 ZIP 仍要重跑 §20（§15.1）。
2. 在 workspace manifest 提升到 `1.0.0`，更新经审核的发布说明；不增加依赖版本锁。
3. 在最终提交上跑适用门禁和两次干净打包。
...
每一批同步对应条款、代码和必要的指南；每批通过 AGENTS.md 的宿主检查后提交。Linux/Android 相关改动另跑 §15.1。不实现 TCX。

| 顺序 | 实施范围 | 合同位置 | 必须证明的行为 |
|---|---|---|---|
| 1 | 合同更正：§5 模块树、§7.4 故障分类、§14.1 `uid_stats`、§12.5.1 插入点、D23 导出半句 | 蓝图、§0.6.18、本文 | 文档与代码一致；doc-check 绿 |
| 2 | `flux-core::runtime`：Phase / Stimulus / step / CommittedView / Generations | §26、§10.1 | `cargo test -p flux-core` 覆盖 §26 表；capture-side drift 不得 `PublishInactive` |
| 3 | reactor 降为适配器；换代事务、控制会话、订阅调度、世代各归所有者 | §9.4、§10.2、§26 | 用户可见 `module.prop` 词表不变；删除带 sleep 的换代旁路 |
| 4 | clsact attach 模块；MapSet 为 maps 所有者；MAP_SPECS 生成 C 占位 | §12.5.1、§8.5 | 无 `trait Attach`；身份删除仍从内核 dump 证明 |
| 5 | `flx_in` 已建立/分片快路径；Decision CREATE 不变量 | §7.5、§14.1 | 非裸 SYN 不 assign；ABI 不 bump |
| 6 | 工作区版本 `1.0.0-rc.2` | §13.4 | 已改；正式 1.0.0 仍等远端 CI 与 §20 |

PHIL-10：机制参数仍内部生成；不新增周期探测；世代只有一处可写；删除仍靠身份。
