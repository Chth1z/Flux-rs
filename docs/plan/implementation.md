...
**正在按所有者新确认的产品设计继续收敛 1.0.0 候选。** 工作区版本暂保留原值；接口收敛后可准备 RC，正式 1.0.0 仍需所有者审核和候选验收。基准为 `aa4715d`，审核分支为 `codex/v1.0.0-review`。`d6be361` 是上一批候选的文档收尾，不代表本轮新增要求已实现。后续 §17.2–§17.12 保留阶段编号与回归方法，不是新的待办清单。
...
| 本轮产品收敛 | **六批范围已实现，软件复验与候选打包中** | 所有者确认 `id=Flux-rs`、统一节点来源、进阶配置、增量升级、运行目录与展示方向；执行顺序见 §17.0.4 |
...
上一批确认的实现缺口已处理。节点输入已提交 `1022e63`，刷新计划与订阅模块职责修正见 §0.6.10；版本解析、构建记录和网络设计见 §0.6.11–§0.6.12。上一批软件与双构建复验见 §0.6.13。**这些结果不覆盖本轮合同修订；新的实现差距集中在 §17.0.4。**
...
**推荐保留当前产品路线，按下面的证据完成验收后发布。** 本轮以所有者重申的“优雅、简洁高效、从根本上避免问题”为取舍标准：优先消除重复真相、调用顺序约束和无效状态；不靠增加兜底分支或重复校验处理内部设计问题。对应 PHIL-1、PHIL-2、PHIL-4、PHIL-6。审查入口是本分支 diff、§10.2 的模块接口与 §0.6.8–§0.6.13 的复现记录。
...
| 3：官方引擎来源 | 干净提交 5fa639c 解析 1.14.1，官方 host 检查、Android 资产校验、双构建与对应源码归档通过 | 提升版本后按同一路径生成正式产物 |
...
| 9：卸载与残留 | 宿主生命周期脚本验证安装器/服务脚本 | 候选卸载并重启后的实际残留检查 |
| 10：三个管理器 | 只有历史 KernelSU 真机证据 | Magisk、KernelSU、APatch 各跑一次完整 smoke |
| 11：可复现打包 | 最终候选在 `/tmp` 下两个独立目标目录完整构建，ZIP 字节一致 | 哈希与范围见 §0.6.13；提升版本后复跑 |
| 12：DNS 精度 | 历史 Q9 记录可查 | 候选验证被选与未选应用的系统 DNS |
| 13：用户边界说明 | 首次启用由现有事务完成验证，`check` 用于诊断；控制面板告警已与合同对齐 | 所有者审核对外表述 |
| 14：实现差距 | 本轮设计已实现；最终软件复验、打包和设备审核范围见 §17.0.4 | 完成各批实现、审核与验证，再审核最终分支 |

本轮软件门禁以 §0.6.15 记录的当次结果为准；§0.6.13 的旧整包验证不覆盖新实现。**WSL 本地通过不等于远端 CI 已通过；旧设备记录不等于这个候选已通过。**
...
现行 README 和指南已同步统一来源、进阶配置、迁移、运行目录与状态展示。Windows 纯逻辑测试、Linux daemon 单元测试和多来源 HTTP 端到端测试已通过；完整软件门禁和候选打包在提交时复验。本轮没有校园网或设备测试环境，不将这些软件结果外推为校园网故障已修复或新设备已通过。
