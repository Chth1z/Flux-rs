...
**缺口 2 — 出厂模板的 `PROXY=["DIRECT"]` 挡住填组。** 见 §0.6.5 第 28 条。原版 Flux 没遇到，是因为它的 `PROXY` 指向空的地区组，不是 `DIRECT`。**处置：模板换回旧版形状，填充规则承认 `DIRECT` 占位。**
