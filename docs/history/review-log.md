...
### 0.6.9 独立节点输入与配置职责（2026-09-15）

**所有者要求。** 手工分享链接可以单独使用，也可以与可选订阅混用；配置按职责保持简洁。所有者随后明确保留 `module/template.json` 原有的 `outbounds`，并取消 sing-box 及任何依赖的版本固定。Astra / low 子代理负责代码，主代理负责设计、复核与分批提交。示例与测试全部使用占位值，未保存或连接对话中的真实节点。

| 原说法 / 实现 | 实际问题 | 处置 |
|---|---|---|
| 节点只从订阅或用户写好的 JSON 获得 | 有手工链接仍要制造订阅来源；缺远端缓存成为启动前置条件 | `[nodes] list` 复用现有 `@file` 规则，配置解析后即是节点值；手工节点和当前 URL 对应的订阅快照进入同一个 `assemble_nodes` 和引擎候选事务 |
| 订阅抓取、重命名和截断都平铺在 `[subscription]` | 远端名称整理与网络参数混在一起，也不应处理用户亲自命名的节点 | 清洗选项移入 `[subscription.refine]`，内部分为 `SubscriptionConfig` / `RefinementConfig`；手工名字保真。无双语法、无自动改写配置，指南说明一次性迁移 |
| 草案把默认 PROXY 改成所有节点菜单 | 超出了所有者保留原菜单的意图 | 在提交前恢复全部原始 outbounds；无地区节点可由用户显式引用或使用已有空 AUTO 语义，生成器仍只填空和追加 |
| 不认识的传输参数静默落成 TCP | 一份外观已转换的配置实际改变了协议 | 未实现的 transport / VLESS encryption 明确报错；HY2 保留完整解码后的 userinfo，支持 hy2 别名，拒绝无法准确表达的 pinSHA256 / mport。此处是外部格式转换边界，不新增运行期兜底 |
| 读取旧缓存早于采用新响应；共享错误是带 token 的字符串 | 无法读取的旧缓存能挡住有效新响应；错误再次包装会丢失 too_large 分类 | pending 响应先行，仅无响应时读取 URL 绑定缓存；共享 `(token, detail)`，检查与运行采用同一选择规则 |
| TOML 语法错误直接显示解析器原文 | 行内节点的凭据会随出错行进入诊断 | 入站解析只生成固定语法说明及行列位置，不复制源文本；不是在日志出口再加脱敏层 |
| 引擎与构建工具要求固定版本 | 与所有者最新版本策略冲突 | §9.7、§13.4 先改为上游解析与实际构建证据；构建实现仍待下一批完成，差距登记于 §17.0.2。本条不声称解除版本固定已实现 |
...
### 0.6.14 所有者审核后的产品设计收敛（2026-09-16）

**这是设计修订记录，尚未交付对应代码。** 所有者确认以 `id=Flux-rs` 作为 Rust 版本独立身份，不要求维护或迁移旧版 shell Flux；认可统一节点来源、可选进阶配置、增量升级、运行产物集中与旧版风格状态展示，并要求进一步考虑适合极客用户的选项。所有者最初指定主代理负责设计、Cursor Grok 4.6 Extra High 执行；控制与下载通道失败后，授权主代理自行执行。没有通过 Cursor 派发实现任务。当前实施范围见 §17.0.4。

| 原说法 | 复核与决定 | 处置 |
|---|---|---|
| 手工 `[nodes]` 与单个 `[subscription]` 分开，进阶配置不另建文件 | 所有者认可用统一来源表达用户意图，并将低频策略分到可选进阶文件 | 就地修改 §11.2、§28.2；每个字段只归一个文件。HTTP 订阅和 HTTP 代理有真实语法重叠，提供显式 node 条目，禁止解析失败后的类型猜测 |
| 默认 whitelist 可在读取失败后退回空配置 | 新默认是 blacklist；若沿用空配置退回，就会扩大应用选择。源码依据：`crates/fluxd/src/checks.rs:115` 的 missing-file 分支与 `crates/flux-core/src/config.rs:240` 的默认值 | §11.2 区分缺失文件与有效空文件；§13.2.3 要求迁移保留旧隐式默认。此处为设计推理，代码待改 |
| 安装器只在文件缺失时复制，已经能够保留升级设置 | 能保留文件，但不能迁移结构；旧版确有配置迁移。源码：`module/customize.sh:92`、`clone/Flux-original/customize.sh:122` | 增加 §13.2.3：一次性 Rust 迁移保留显式值、旧语义与原始备份；守护进程继续只读配置 |
| `subscription.raw` 配合 `subscription.url` 绑定来源 | 两个文件只为表达一项来源关系，URL 标记、提交顺序与失效清理分散在 reactor。源码：`crates/fluxd/src/reactor.rs:3130`、`:3261` | §28.5 使用来源标识作为缓存路径，只保留原始响应；保留离线重建能力，取消双文件绑定机制 |
| 日志在根目录，超过 4 MiB 时启动轮转 | 程序长时间运行时不会因此轮转。源码：`crates/fluxd/src/reactor.rs:4170`；引擎工作目录也是数据根，见 `crates/fluxd/src/engine.rs:177` | 默认产物集中 `run/`，日志大小与保留份数成为进阶策略并在写入时执行；缺省 engine cache 路径在生成时补全，显式路径保留 |
...
### 0.6.22 PolicyEpoch 双 bank 落地（2026-09-17）

**原说法：** §10.5 已禁止 add-then-subtract，但 `apply_policy` 仍先加后减；E1 未选路径不读 control。
