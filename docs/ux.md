...
### 3.3 `external_ui_download_url` 的取舍

`external_ui_download_url` 让 sing-box 在运行时下载 zashboard。方便，但它是一个**运行时网络取回 + 解压到本地目录**的动作，即一条供应链路径。

两个选项，倾向第一个：

- **（倾向）模板里写好 URL，但默认不自动下载**，由用户在 zashboard 里点一次或跑一次 `fluxd ui fetch`。让"下载第三方 UI"成为一个显式动作。
- 打包时把 zashboard 一起塞进模块。模块体积增加，且我们要为它的版本与安全负责。
...
订阅格式转换（Clash YAML → sing-box JSON 之类）是一个**没有底的维护坑**：格式会漂移、机场各有私有字段、sing-box 自己的 schema 也在演进。把它放进 `fluxd` 意味着我们要长期追这三条曲线。

**建议的做法**：`fluxd subscribe <url>` 只做四件事——取回、`sing-box check` 校验、原子替换、触发候选切换。转换由 URL 那一端负责：

- 大多数机场支持 `&target=singbox` 之类的参数直接返回 sing-box 配置。
- 或者用户自建/指定一个 subconverter，`fluxd` 只认最终的 sing-box JSON。

这样 `fluxd subscribe` 是约 200 行的确定性代码，而不是一个格式适配层。
...
1. **zashboard 的分发方式**：§3.3 的两个选项（默认不自动下载 / 打包进模块）。
2. **`explain` 的深度**：现在会把实测数字（"发出 12043，收到 0"）直接给用户。信息量大，但也可能显得吓人。
3. **是否接受"订阅不做转换"**（§4.1）。如果坚持要内置转换，我建议它是一个独立的可选二进制，不进 `fluxd`。
