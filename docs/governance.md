...
本文规定 Flux-rs 怎么被开发和维护。它约束的是**过程**，不是技术方案——技术合同是 `docs/spec/blueprint.md`（0.9.5 折叠完成前，还要叠 `blueprint-0.9.1.md` 与 `blueprint-0.9.2.md` 两层增量，冲突时后者优先）。
...
- **代码只引用 `§N.N`，不引用版本化的 `R09x-NN`。** 前者跟着内容走、活得过再版；后者随折叠退役。0.9.5 折叠时把代码里现存的 `R09x` 引用改写成承载它的 §。
...
| 实践 | 当前做法 | 为什么 |
|---|---|---|
| **精确归档 allowlist** | `xtask/src/package.rs` 只允许 R091-12 规定的 14 个文件，意外文件使打包失败 | 供应链边界可枚举、可测试 |
