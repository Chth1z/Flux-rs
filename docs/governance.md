...
本文规定 Flux-rs 怎么被开发和维护。它约束的是**过程**，不是技术方案——0.9.1 技术合同由冻结的 `docs/spec/blueprint.md`（0.9.0 基线）与 `docs/spec/blueprint-0.9.1.md`（增量修订）共同构成，冲突时后者优先。
...
3. 在 `history/review-log.md` §0.6 的对照表里加一行：**原说法 / 实际 / 处置**。
...
| 文件 | 是什么 | 谁改 |
|---|---|---|
| `docs/spec/blueprint.md` | **冻结的 0.9.0 规范性基线**，只读保留 | 不再修改 |
| `docs/spec/blueprint-0.9.1.md` | **0.9.1 规范性增量**；与基线冲突时优先 | 已定稿，不再修改 |
| `docs/spec/blueprint-0.9.2.md` | **0.9.2 规范性增量**；与前两层冲突时优先 | 0.9.2 设计变更时 |
