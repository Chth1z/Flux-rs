...
- 基线：[`blueprint.md`](blueprint.md)，文档编号 `FLUX-BP-0.9.0-FINAL`
...
| 主题 | 当前真相源 |
|---|---|
| 0.9.1 产品行为、状态机、模块接口 | `blueprint.md` + 本文；冲突时本文优先 |
...
| 历史推翻与研究依据 | `evidence/review-log.md` |

`architecture.md`、`introduction.md`、`ux.md` 与 `reference/` 是当前合同的读者投影。它们必须标明目标版本；“以蓝图为准”不是保留冲突的许可证。

---

## R091-01：把 fail-open 改为 admission-bounded 合同

**覆盖**：0.9.0 §2.2、§20；`introduction.md` 中“任何失败都不会断网”的绝对表述。
...
**覆盖**：0.9.0 §1.3、§13；`ux.md` §1.3/§3/§4/§5；`introduction.md` 的订阅与 zashboard 默认体验。

0.9.1 范围如下：

- **包含** `action.sh`。它只是 `disable` 文件的前端：先读状态，再明确调用 `enable` 或 `disable`，不保存第二份 toggle 状态。
...
- 默认 `sing-box.json` 只有 direct outbound、direct final route、`sniff` 与 `hijack-dns` 两条必要规则；无远程内容。
- Flux 永不反写 `config/flux.toml` 或 `config/sing-box.json`。安装和升级只在文件缺失时复制 bootstrap 默认值。

远程 subscription 继续作为延期项，但只有实际加入第二种配置来源时才允许定义转换模块与网络 seam。不得为它预建未使用的 trait、schema registry 或协议字段。

## R091-04：路径、schema 与首次安装状态只有一套

**覆盖**：0.9.0 §11、§13；`ux.md` 与 `introduction.md` 的旧路径/字段。
...
安装器先创建两份配置（仅当缺失），再创建 `disable`。因此 fresh install 的状态是 **Disabled 且已生成 bootstrap 配置**，不是“未配置”。用户编辑两份 authority file，运行 `fluxd check`，再显式 enable。

## R091-05：TC preference、可达性与 `clsact` 所有权

**覆盖**：0.9.0 §3.4、§8.5、§8.7、§8.9、§12.5、§26；`architecture.md` 的固定 pref 1；所有“必须是首个 classifier”的表述。
...
实际 pref 必须进入所有权记录与 status。0.9.1 在 `ifaces[]` 增加可选数字字段 `pref`；这是 additive status 字段，不删除/改名既有 JSON 字段，也不需要为唯一 CLI adapter 引入独立 wire version。禁止缓存“整台设备的可用 pref”；同一设备的 Wi-Fi 与 rmnet 可以不同。
...
`first_applicable` 这个既有 JSON 字段名在 0.9.1 **保留以避免无关 wire 破坏**，但其合同含义是“该 filter 在 admission 时通过可达性验证且前置 filter 快照未漂移”，不是“dump 中排第一”。人类输出统一写“reachable / 可达”。未来若出现真正的第二个独立客户端，再在一次显式协议升级中改名。
...
**覆盖**：`architecture.md` 的“root cgroup 当前占满”与 Phase 0 实测冲突。
...
**覆盖**：0.9.0 §9.0、§9.1 中残留的旧地址说明；`verification/phase0.md` Q2 的“同端口”旧形状。
...
**覆盖**：0.9.0 §8.8、§10、§13.2、§20、§26；`ux.md` 与 `introduction.md` 的“立即拆除/立即清空”。

| 操作 | 同步完成的事 | 不做的事 |
|---|---|---|
| `disable` | 创建开关；publish `active=0`；终止 engine；daemon 继续运行 | 不 detach TC、不删 veth/rule/route/map |
| `stop` | publish `active=0`；终止 engine；daemon 退出 | 不 flush、不承诺当前 boot 零内核对象 |
| `uninstall.sh` | 先 `disable`，再 `stop`，删除 `/data/adb/flux-rs` | 不扫描相似路径，不 flush 系统/Flux 网络对象 |
...
**覆盖**：0.9.0 §10.3/§10.6；`ux.md` 的版本化协议、`explain`、`watch`、`subscribe` 清单。
...
- ZIP 内容由 `xtask/src/package.rs` 的精确 15 文件 allowlist 决定。
...
5. 更正表的唯一位置是 `evidence/review-log.md` §0.6；governance/authoring 不再指向已移出的 `blueprint.md` §0.5。
6. 规范性技术合同是 0.9.0 基线 + 本增量；`architecture.md` 只是导览，不是第二份权威合同。
...
**覆盖**：`governance.md` §4.1 的无条件 `cargo test --workspace`；本文验收中“任意主机都跑全 workspace”的隐含要求。
...
| `action.sh` 有/无 | 有，且只是开关前端 | R091-03 |
| subscription 0.9.x 有/无 | 0.9.1 无 | R091-03 |
| 默认 clash_api/zashboard 下载有/无 | 默认无，用户可自行配置 | R091-03 |
...
| `webroot` 计划 vs package 禁止 WebUI | 禁止 `webroot` | R091-03、R091-12 |
...
| 更正表位置 §0.5/§0.6 | `evidence/review-log.md` §0.6 | R091-13 |
...
1. `docs/blueprint.md` 的 clean-filter blob 与 0.9.0 `HEAD` 保持一致；
